...
import { FileUp, FileText, Orbit, X, Check, Loader2, ArrowRight } from "lucide-react"
...
import { demoObservations } from "@/lib/demo-data"
...
export function UploadWorkspace() {
  ...
  const runPipeline = useCallback((parsed: ParsedFile) => {
    setFile(parsed)
    setStep("parsing")
    setProgress(10)
  ...
  const handleFile = useCallback(
    (fileList: FileList | null) => {
      ...
      const rows = Math.floor(800 + Math.random() * 4200)
      const parsed: ParsedFile = {
        name: target.name,
        sizeLabel: formatBytes(target.size),
        rows,
        columns: ["time_bjd", "relative_flux", "flux_error", "channel"],
      }
      runPipeline(parsed)
    },
    [runPipeline],
  ...
  const selectArchiveObservation = useCallback(
    (id: string) => {
      ...
      const obs = demoObservations.find((o) => o.id === id)
      if (!obs) return
      const rows = Math.floor(2400 + Math.random() * 1800)
      runPipeline({
        name: `${obs.name}.fits`,
        sizeLabel: "Archive dataset",
        rows,
        columns: ["time_bjd", "relative_flux", "flux_error", "channel"],
      })
    },
      ...
      <Card className="border-border/60 bg-card/60">
        ...
        <CardContent>
          <Tabs defaultValue="file">
              ...
              >
                ...
                <div className="flex flex-col gap-1">
                  <p className="text-sm font-medium text-foreground">
                    Drag and drop a CSV or FITS-style file
                  </p>
                  <p className="text-xs text-muted-foreground">
                    Expected columns: time_bjd, relative_flux, flux_error, channel
                  </p>
                </div>
                <Button
                  size="sm"
            ...
            <TabsContent value="archive" className="flex flex-col gap-2 pt-4">
              {demoObservations.map((obs) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <Orbit className="size-4 text-primary" strokeWidth={1.75} />
        ...
        <CardContent className="flex flex-col gap-5">
          {!file && (
            <p className="text-sm leading-relaxed text-muted-foreground">
              Upload a file or pick an archive entry to see cadence validation and a
              light-curve preview before running inference.
            </p>
          )}
              ...
              {step === "ready" && (
                <div className="flex flex-col gap-3 border-t border-border/60 pt-4">
                  <div className="grid grid-cols-2 gap-3 font-mono text-xs text-muted-foreground">
                    <span>
                      Columns:{" "}
                      <span className="text-foreground">{file.columns.length}</span>
                    </span>
                    <span>
                      Cadence: <span className="text-foreground">nominal</span>
                    </span>
                  </div>
                  <LightCurvePreview seed={file.rows} />
                  <Button className="mt-1" onClick={() => router.push("/inference")}>
